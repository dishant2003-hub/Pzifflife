...
$(function($) {
  ...
  function MBox(selector, clicked, options = null) {
    ...
    this.updateVars();
    this.createNavigation(this.imagesCount);
  ...
  MBox.prototype = {
    ...
    build: function() {
      ...
      if(this.imagesCount > 1) {
        ...
        $('<i/>', {
          'class': 'fa fa-chevron-circle-left point mBox-previous float-left'
        }).appendTo(navigationArrows);

        $('<i/>', {
          'class': 'fa fa-chevron-circle-right point mBox-next float-right'
        }).appendTo(navigationArrows);
        ...
        if(this.settings.displayThumbnails) {
          ...
          this.images.each(function() {
            ...
            //$(this).wrap('<div class = "mBox-image" style = "width: 120px;"></div>');
            $(this).wrap('<div class = "mBox-image"></div>'); // @ size of the tile should be a size of images per page.
    ...
    createNavigation: function(imagesCount) {
      ...
      $('<span />', {
        ...
        'text': (this.currentImageNo + 1)+"/"+imagesCount
      }).prependTo("#mBox");
      ...
      if(this.imagesCount > 1) {
        ...
        navigationCount.append(' of ' + this.imagesCount); 

        //$(".mBox-current-image-no").text(this.imagesCount)// Add to the end.
        ...
        if(this.imagesCount > this.settings.imagesPerPage) {
          ...
          for(var i = 0; i < totalPages; i++) {
            var circ = $('<i/>', {
              'class': 'fa fa-circle point circle_show',
              'data-page-number': i
    ...
    navigationUpdate: function(no) {
      ...
      $('.mBox-current-image-no').text(no + 1 +"/"+ this.imagesCount) ;
    ...
    addEventListeners: function() {
      ...
      $('button.mBox-close').on('click', $.proxy(function() {
        this.close();
        $(document.body).removeClass('mbox-hide');
      }, this));
